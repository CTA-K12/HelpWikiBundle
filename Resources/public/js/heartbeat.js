...
( function( $, window, undefined ) {
    var Heartbeat = function() {
        ...
        function initialize() {
            ...
            if ( hidden ) {
                ...
                $document.on( visibilitychange + '.mesd-heartbeat', function() {
            ...
            $(window).on( 'unload.mesd-heartbeat', function() {
        ...
        function connect() {
            ...
            settings.xhr = $.ajax({
                ...
                if ( response.csfr_expired ) {
                    $document.trigger( 'heartbeat-csfr-expired' );
                    return;
                }
        ...
        function userIsActive() {
            ...
            $document.off( '.mesd-heartbeat-active' );

            $('iframe').each( function( i, frame ) {
                if ( isLocalFrame( frame ) ) {
                    $( frame.contentWindow ).off( '.mesd-heartbeat-active' );
                }
            });
        ...
        function checkUserActivity() {
            ...
            if ( ! settings.userActivityEvents ) {
                $document.on( 'mouseover.mesd-heartbeat-active keyup.mesd-heartbeat-active touchend.mesd-heartbeat-active', function() {
                    userIsActive();
                });

                $('iframe').each( function( i, frame ) {
                    if ( isLocalFrame( frame ) ) {
                        $( frame.contentWindow ).on( 'mouseover.mesd-heartbeat-active keyup.mesd-heartbeat-active touchend.mesd-heartbeat-active', function() {
                            userIsActive();
                        });
                    }
                });
         ...
         * Use mesd.heartbeat.isQueued('handle') to see if any data is already queued for that handle.
         *
    ...
    // Ensure the global `mesd` object exists.
    window.mesd = window.mesd || {};
    window.mesd.heartbeat = new Heartbeat();
